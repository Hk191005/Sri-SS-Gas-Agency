...
export const Payments: React.FC = () => {
  ...
  return (
    <div className="space-y-6 max-w-7xl mx-auto">
      ...
      {/* Summary KPI Bar — Pure White Cards */}
      <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
        <div className="saas-card bg-white dark:bg-[#171717] border border-[#E5E5E5] dark:border-[#2A2A2A] p-5 rounded-2xl shadow-xs flex items-center justify-between">
          <div>
            <span className="text-[11px] font-black uppercase tracking-wider text-[#525252] block">Customer Collections Logged</span>
            <span className="text-2xl font-black text-[#16A34A] dark:text-emerald-400 mt-1 block">₹{totalCollected.toLocaleString('en-IN')}</span>
          </div>
          <div className="w-10 h-10 rounded-xl bg-[#F0FDF4] text-[#16A34A] border border-emerald-200/60 flex items-center justify-center shrink-0">
        ...
        <div className="saas-card bg-white dark:bg-[#171717] border border-[#E5E5E5] dark:border-[#2A2A2A] p-5 rounded-2xl shadow-xs flex items-center justify-between">
          <div>
            <span className="text-[11px] font-black uppercase tracking-wider text-[#525252] block">Security Deposits Held</span>
            <span className="text-2xl font-black text-[#171717] dark:text-white mt-1 block">₹{totalDeposits.toLocaleString('en-IN')}</span>
          </div>
          <div className="w-10 h-10 rounded-xl bg-[#FAFAFA] text-[#171717] border border-[#E5E5E5] flex items-center justify-center shrink-0">
        ...
        <div className="saas-card bg-white dark:bg-[#171717] border border-[#E5E5E5] dark:border-[#2A2A2A] p-5 rounded-2xl shadow-xs flex items-center justify-between">
          <div>
            <span className="text-[11px] font-black uppercase tracking-wider text-[#525252] block">Supplier Payables (SUPERGAS)</span>
          ...
          <div className="w-10 h-10 rounded-xl bg-[#FFFBEB] text-[#D97706] border border-amber-200/60 flex items-center justify-center shrink-0">
      ...
      <div className="flex items-center gap-2 border-b border-[#F1F1F1] dark:border-[#262626] pb-2 overflow-x-auto text-xs font-extrabold">
        {[
          ...
          { id: 'payments', label: 'Payment Receipts Log' },
          { id: 'deposits', label: 'Held Security Deposits' },
      ...
      {loading ? (
        ...
        <div className="saas-card bg-white dark:bg-[#171717] p-8 text-center rounded-2xl border border-red-200 dark:border-red-900/50 space-y-3">
          ...
          <p className="font-black text-red-600 dark:text-red-400 text-sm">Failed to load financial registers</p>
              ...
              {receivablesCustomers.length === 0 ? (
                ...
                receivablesCustomers.map(({ customer, outstanding }) => (
                  <tr key={customer.id} className="hover:bg-[#FAFAFA] dark:hover:bg-[#1F1F1F] transition-colors">
                    ...
                    <td className="py-3.5 px-4 font-black">{customer.name}</td>
              ...
              {payments.length === 0 ? (
                ...
                payments.map((p) => (
                  <tr key={p.id} className="hover:bg-[#FAFAFA] dark:hover:bg-[#1F1F1F]">
                    ...
                    <td className="py-3.5 px-4 font-mono font-bold text-[#737373]">{p.customer_id.substring(0, 8)}</td>
                    <td className="py-3.5 px-4 capitalize font-bold text-[#525252]">{p.payment_method}</td>
          ...
          <table className="w-full text-left text-xs font-semibold">
            <thead className="bg-[#FAFAFA] dark:bg-[#1F1F1F] text-[#525252] uppercase text-[10px] font-extrabold tracking-wider border-b border-[#E5E5E5] dark:border-[#2A2A2A]">
              <tr>
                ...
                <th className="py-3.5 px-4">Customer ID</th>
                <th className="py-3.5 px-4">Cylinder Size</th>
              ...
              {deposits.length === 0 ? (
                ...
                deposits.map((d) => (
                  <tr key={d.id} className="hover:bg-[#FAFAFA] dark:hover:bg-[#1F1F1F]">
                    <td className="py-3.5 px-4 font-mono font-black text-[#171717]">DEP-{(d.id || '').slice(0, 8).toUpperCase()}</td>
                    <td className="py-3.5 px-4 font-mono font-bold text-[#737373]">{d.customer_id.substring(0, 8)}</td>
                    <td className="py-3.5 px-4 font-bold">{d.notes || '12 kg'}</td>
                    <td className="py-3.5 px-4 font-black text-[#171717]">₹{d.amount.toLocaleString('en-IN')}</td>
              ...
              {supplierPayables.length === 0 ? (
                ...
                supplierPayables.map((sp) => (
                  <tr key={sp.id} className="hover:bg-[#FAFAFA] dark:hover:bg-[#1F1F1F]">
                    ...
                    <td className="py-3.5 px-4 font-black text-[#171717]">₹{(sp.total_amount || 0).toLocaleString('en-IN')}</td>
